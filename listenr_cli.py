...
def get_lemonade_ws_url() -> str:
    ...
    try:
        ...
        print(f"WARNING:  Could not discover Lemonade websocket port ({e}), using default :8001")
...
def ensure_models_loaded(debug: bool = False) -> None:
    ...
    """
    print(f"Loading Loading Whisper model '{WHISPER_MODEL}' in Lemonade...", flush=True)
    try:
        result = lemonade_load_model(WHISPER_MODEL)
        print(f"OK: Whisper: {result.get('message', result)}")
    ...
    if USE_LLM:
        print(f"Loading Loading LLM '{LLM_MODEL}' in Lemonade...", flush=True)
        try:
            result = lemonade_load_model(LLM_MODEL)
            print(f"OK: LLM: {result.get('message', result)}")
        except requests.HTTPError as e:
...
async def main(save: bool, show_raw: bool, debug: bool):
    ...
    print(f"Listenr Listenr CLI — streaming to Lemonade")
    ...
    async for result in asr.stream_transcribe(
        ...
        if msg_type == 'conversation.item.input_audio_transcription.delta':
            ...
            if is_hallucination(raw_text):
                ...
                if debug:
                    print(f"  [DEBUG] SKIP: hallucination dropped: {raw_text!r}", flush=True)
                pcm_buffer.clear()
                continue
            ...
            if not stripped_text:
                print('\r' + ' ' * 80 + '\r', end='', flush=True)
                if debug:
                    print(f"  [DEBUG] SKIP: all-noise stripped: {raw_text!r}", flush=True)
                pcm_buffer.clear()
                continue
            if stripped_text != raw_text and debug:
                print(f"  [DEBUG] NOTE: noise tags stripped: {raw_text!r} → {stripped_text!r}", flush=True)
            ...
            if save:
                if pcm_buffer:
                    ...
                    print(f"  WARNING:  [SAVE SKIPPED] pcm_buffer is empty — no audio captured for this segment", flush=True)

        elif msg_type == 'input_audio_buffer.speech_started':
            if debug:
                print(f"  [DEBUG] speech:  speech_started — clearing pcm_buffer (had {len(pcm_buffer)} chunks)", flush=True)
            pcm_buffer.clear()  # start fresh buffer for this segment

        elif msg_type == 'error':
            print(f"  ERROR: Error from Lemonade: {result.get('message', result)}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\nLoading Unloading models from Lemonade...", flush=True)
        result = lemonade_unload_models()
        if 'error' in result:
            print(f"  WARNING:  Unload failed: {result['error']}")
        else:
            print(f"  OK: Models unloaded.")
        print("👋 Stopped.")
